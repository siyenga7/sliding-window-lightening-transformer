...
class QuantAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 ...
                 enable_wdm_noise=False, num_wavelength=9, channel_spacing=0.4, enable_linear_noise=False, window_size=4):
        super().__init__()
    ...
    def create_sliding_window_mask(self, seq_len, window_size, device):
    ...
    def forward(self, x):
        ...
        if not self.training and self.phase_noise_std > 1e-5:
            ...
            sliding_window_mask = self.create_sliding_window_mask(N, self.window_size, q.device)
            attn = attn_scores.masked_fill(sliding_window_mask.unsqueeze(0).unsqueeze(0) == 0, float('-inf')) 
        else:
